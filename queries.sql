...
-- Query 33: Approve an order
UPDATE orders SET status = 'approved' WHERE order_id = 3;

-- Query 34: Reject an order
UPDATE orders SET status = 'rejected' WHERE order_id = 3;

-- Query 35: Get equipment info for rejected order
SELECT equipment_id, order_quantity FROM orders WHERE order_id = 3;

-- Query 36: Restore stock for rejected order
UPDATE equipment SET stock = stock + 5 WHERE equipment_id = 1;
